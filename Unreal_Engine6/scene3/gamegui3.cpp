...
//    m[1] = orientation->operator()(1,0);
//    m[2] = orientation->operator()(2,0);
//    m[3] = 0;
//    m[4] = orientation->operator()(0,1);
//    m[5] = orientation->operator()(1,1);
//    m[6] = orientation->operator()(2,1);
//    m[7] = 0;
//    m[8] = orientation->operator()(0,2);
//    m[9] = orientation->operator()(1,2);
//    m[10] = orientation->operator()(2,2);
//    m[11] = 0;
//    m[12] = 0;
//    m[13] = 0;
//    m[14] = 0;
//    m[15] = 1;
    float w = pactorList->getActor(id)->getRigidbody()->getOrientation()->getW();
    float x = pactorList->getActor(id)->getRigidbody()->getOrientation()->getI();;
    float y = pactorList->getActor(id)->getRigidbody()->getOrientation()->getJ();;
    float z = pactorList->getActor(id)->getRigidbody()->getOrientation()->getK();;
    m[0] = 1 - (2*y*y + 2*z*z);
    m[4] = 2*x*y + 2*z*w;
    m[8] = 2*x*z - 2*y*w;
    m[1] = 2*x*y - 2*z*w;
